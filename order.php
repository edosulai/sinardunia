...
$itungtrans = mysqli_query($conn, "SELECT count(detailid) as jumlahtrans, idproduk from detailorder WHERE orderid='$orderidd'");
$itungtrans2 = mysqli_fetch_assoc($itungtrans);
$itungtrans3 = $itungtrans2['jumlahtrans'];

$orders = mysqli_query($conn, "SELECT * FROM detailorder WHERE orderid='$idorder'");
...
if (isset($_POST['selesai'])) {
	$updatecartstatus = mysqli_query($conn, "UPDATE cart set status='Selesai' WHERE orderid='$idorder'");
	$updateprodukstatus = mysqli_query($conn, "UPDATE produk set stok=stok-$qty WHERE idproduk='$idproduk'");

	if ($updatecartstatus && $updateprodukstatus) {
		...
		<div class="container">
			...
			<div class="checkout-left">
				...
				<div class="checkout-right-basket">
					<?php
					$fetc = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM cart WHERE orderid='$idorder' and status='Selesai'"));
					if (!isset($fetc)) { ?>
						<form method="post">
							<input type="submit" name="selesai" class="btn btn-success" value="Pesanan Diterima" \>
						</form>
					<?php } ?>
