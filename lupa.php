...
if (isset($_POST['lupa'])) {
  ...
  if (mysqli_num_rows($cek) > 0) {
    ...
    $mgClient = Mailgun::create('d39368a4df57da33f22264a1a8ecd50b-8d821f0c-c6fa0991');
    $domain = "sandbox3cb07770db1044d4830b0796f53a6d55.mailgun.org";

    $mgClient->messages()->send($domain, array(
      'from'    => 'Sinardunia <mailgun@' . $domain . '>',
      'to'      => $d->namalengkap . ' <' . $email . '>',
      'subject' => 'Reset Password',
      'text'    => 'Silahkan klik link berikut untuk reset password anda : <a>http://localhost/Sinardunia/reset.php?token=' . base64_encode($d->password) . '</a>'
    ));

    echo '<script>alert("Link ganti password sudah dikirim ke email anda.");window.location="login.php"</script>';
  } else {
    ...
    <div class="container">
      ...
      <div class="login-form-grids animated wow slideInUp" data-wow-delay=".5s">
        <form method="post">
          ...
          <input type="submit" name="lupa" value="Lupa">
        </form>
      </div>
      <div>
        <h4>Sudah daftar?</h4>
